                  ...
                  try {
                    ...
                    if (googleTokenRequiredTools.has(callName) && !googleTokenRef.current) {
                      ...
                      if (!waPermissions.generate_image) {
                        ...
                        // Return immediately so Gemini doesn't block
                        result = { ok: true, title: prompt.substring(0, 50), content: '' };

                        // Fire-and-forget: runs in background, survives component unmount
                        const uid = user.uid;
                        const gFetchRef = gFetch;
                        const gToken = googleTokenRef.current;
                        generateImageWithGemini({ prompt, aspectRatio }).then(async (imageResult) => {
                          ...
                          if (gToken) {
                            uploadToDrive(gFetchRef, wsOutput).then(driveResult => {
                              if (driveResult) saveOutput({ ...wsOutput, driveFileId: driveResult.fileId, driveLink: driveResult.link });
                            }).catch(() => {});
                          }
                          ...
                          // Update viewer only if still mounted
                          if (_isMountedRef.current) {
                            setActiveDocument({ title: prompt.substring(0, 50), content: imageHtml, fileType: 'html' });
                            setShowDocumentViewer(true);
                          }
                        }).catch((e: any) => {
                      ...
                      if (!waPermissions.generate_video) {
                        ...
                        // Return immediately so Gemini doesn't block
                        result = { ok: true, title: 'Video Generation', content: 'Starting video generation in background — your video will appear when ready.' };

                        // Fire-and-forget: runs in background, survives component unmount
                        const uid = user.uid;
                        generateVideoWithVeo(prompt).then(async (videoResult) => {
                          ...
                          // Always save to workspace (persists after navigation)
                          await saveOutput({
                            id: `vid_${generationTaskId}`,
                            userId: uid,
                            type: 'video',
                            title: prompt.substring(0, 50),
                            textContent: videoHtml,
                            mimeType: 'text/html',
                            fileSize: new Blob([videoHtml]).size,
                            createdAt: new Date().toISOString(),
                          });
                      ...
                      if (!waPermissions.create_document) {
                        ...
                        // Return immediately so Gemini doesn't block
                        result = { ok: true, title, content: '' };

                        // Fire-and-forget: runs in background, survives component unmount
                        const uid = user.uid;
                        const gToken = googleTokenRef.current;
                        const gFetchRef = gFetch;
                        const lang = authLanguage;
                        const pName = personaName;
                        const histCtx = historyContextRef.current;
                        generateDocumentWithGemini({
                          ...
                          if (!content) throw new Error('No content returned from document generation');
                          ...
                          console.error('Background document generation failed:', e);
                          if (_isMountedRef.current) {
