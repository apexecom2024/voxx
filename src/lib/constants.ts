...
CONVERSATION INTRO:
${locationEnabled ? `Location onboarding has already been handled before the main page. Do NOT call get_user_location during the opening greeting or just because a new voice session started. Use the persistent USER LOCATION DETAILS injected into the system instruction for time-of-day greetings. Only call get_user_location when the user explicitly asks for location-dependent help and you need fresh location context.` : `The user has NOT enabled location access. Do NOT call get_user_location. Instead, greet the user warmly with a neutral greeting (e.g. "Hey there!" or "Hello!") without assuming time of day. If the user later asks something location-dependent (weather, nearby places, etc.), tell them: "I'd love to help with that, but I need your location to do so. You can enable it anytime in your App Settings under the Location section."`}
